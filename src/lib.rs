...
impl<'a> MFRC522<'a> {
     ...
     **/
    #[inline]
    pub fn uninitialized(bus: &'a mut dyn MFRC522Bus) -> Self {
        MFRC522 { bus: bus }
    }
    ...
    pub fn pcd_mifare_transceive(&mut self, data: &[u8], timeout_is_success: bool) -> Status {
        ...
        let (status, nread) = self.pcd_communicate_with_picc(
            pcd::Cmd::Transceive,
            wait_irq,
            tx_buffer,
            Some(rx_buffer.as_mut()),
            &mut valid_bits,
            0,
            false,
        );
    ...
    pub fn pcd_communicate_with_picc(
        ...
        log::debug!("TX |{}.{}|: {:?}.", send.len(), last_bits, send);
        ...
        loop {
            ...
            i -= 1;
            if i == 0 {
                log::warn!("Timeout waiting for TimerIRq.");
                return (Status::Timeout, nread);
            }
        }
        ...
        if let Some(recv) = recv {
            ...
            bus!(
                self.register_read_to_slice(Reg::FIFOData, &mut recv[..n], rx_align),
                return nread
            );
            nread = n;
    ...
    pub fn picc_is_new_card_present(&mut self) -> Option<ATQA> {
        ...
        match status {
            Status::Ok | Status::Collision => Some(atqa),
            _ => None,
        }
    }
    ...
    pub fn picc_select(&mut self, uid: &mut UID) -> Status {
        ...
        loop {
            ...
            log::trace!(
                "picc_select(): cascade level: {}, uid_bits_len: {} tx_uid_bits: {}",
                cascade_level,
                uid_bits_len,
                tx_uid_bits
            );
            ...
            if tx_uid_bits > 0 {
                ...
                let tx_uid_len = tx_uid_bits / 8 + if tx_uid_bits % 8 > 0 { 1 } else { 0 };
            ...
            'anti_collision: loop {
                ...
                log::debug!(
                    "picc_select(): CL: {}, select: {}, tx_uid_bits: {}, tx_len: {}.",
                    cascade_level,
                    select,
                    tx_uid_bits,
                    tx_len
                ...
                let (trx_status, nread) = self.pcd_transceive_data(
                    &tx_buffer[..tx_len],
                    Some(rx_buffer.as_mut()),
                    &mut last_bits,
                    rx_align,
                ...
                if trx_status != Status::Ok && trx_status != Status::Collision {
                    ...
                    if trx_status == Status::Collision {
                        ...
                        let byte_idx =
                            (tx_uid_bits / 8 + 1 + if bit_pos != 0 { 1 } else { 0 }) as usize;
    ...
    fn picc_reqa_or_wupa(&mut self, command: picc::Cmd) -> (Status, ATQA) {
        ...
        let (status, nread) =
            self.pcd_transceive_data(data, Some(buffer_atqa.as_mut()), &mut valid_bits, 0, false);
    ...
    pub fn crc_calculate(&mut self, data: &[u8]) -> Result<u16, Status> {
        ...
        log::debug!("crc_calculate(): {:?} -> {:?}", data, ret);
     ...
     **/
    #[inline]
    pub fn register_read_to_slice(
        &mut self,
        reg: Reg,
        buf: &mut [u8],
        rx_align: u8,
    ) -> bus::MultiResult<usize> {
